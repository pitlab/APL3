...
uint8_t InicjujFiltrKalmanaWysokości4D(void)
{
	...
	fZ[0] = fX[0] = 0.1;	//wysokość
	fZ[0] = fX[1] = 0.01;	//prędkość pionowa
	fZ[0] = fX[2] = 0.001;	//przyspieszenie względne w osi Z
	fZ[3] = fX[3] = AKCEL1G;	//bias osi Z akcelerometru
	arm_mat_init_f32(&mX, 4, 1, fX);
	arm_mat_init_f32(&mZ, 4, 1, fZ);
	
	//wektor szumu procesu
	fW[0] = 0.05;		//odchylenie standardowe błędu pomiaru wysokości
	fW[1] = 0.005;		//odchylenie standardowe błędu pomiaru prędkości pionowej
	fW[2] = 0.0005;		//odchylenie standardowe błędu pomiaru przyspieszenie wzglednego w osi Z
...
uint8_t AktulizacjaWysokościiPrzyspieszeniaFiltraKalmanaWysokości4D(stWymianyCM4_t *dane)
{
	...
	cBłąd |= arm_mat_mult_f32(&mHah, &mX, &mTempW1);
