...
uint8_t DekodujNMEA(uint8_t chDaneIn)
{
    ...
    case ST_GGA_HDOP:	//GPS zwraca puste pole
    	...
    	{
    		 if (chBajtStanu > 3)
    			 cBłąd = DecodeFloat(chBufStanu, chBajtStanu-1, (float*)&uDaneCM4.dane.stGnss1.fHdop);
    ...
    case ST_RMC_LATITUDE_GS:   //długość geograficzna
        if (chDaneIn == ',')
        {
            if (chRMC_Status == 'A')
            {
                cBłąd = DecodeLonLat(chBufStanu, chBajtStanu, &dLatitudeRAW);
                if (cBłąd)
                {
                    chStan = ST_ERR;
                    chBajtStanu = 0;
    ...
    case ST_RMC_LONGITUD_GS:   //szerokość geograficzna
        if (chDaneIn == ',')
        {
            if (chRMC_Status == 'A')
            {
                cBłąd = DecodeLonLat(chBufStanu, chBajtStanu, &dLongitudeRAW);
                if (cBłąd)
                {
                    chStan = ST_ERR;
                    chBajtStanu = 0;
