...
//  = wysokościami z dwu odbiorników GNSS
//  - wysokością z dalmierza laserowego
//  - wysokością z mapy
...
static float32_t fRg1[KPGNS][KPGNS];		//macierz kowariancji pomiaru wysokości z GNSS1
static float32_t fRg2[KPGNS][KPGNS];		//macierz kowariancji pomiaru wysokości z GNSS2
static float32_t fRl[KPLID][KPLID];			//macierz kowariancji pomiaru wysokości z lidaru
static float32_t fRm[KPMAP][KPMAP];			//macierz kowariancji pomiaru wysokości z mapy
static float32_t fQ[KSTAN][KSTAN];			//macierz szumu procesu
static float32_t fI[KSTAN][KSTAN];			//macierz jednostkowa
static float32_t fHc1[KPCIS][KSTAN];		//macierz obserwacji wysokości i prędkości z czujnika ciśnienia 1
static float32_t fHc2[KPCIS][KSTAN];		//macierz obserwacji wysokości i prędkości z czujnika ciśnienia 2
...
static float32_t fHg1[KPGNS][KSTAN];		//macierz obserwacji wysokości z GNSS 1
static float32_t fHg2[KPGNS][KSTAN];		//macierz obserwacji wysokości z GNSS 2
static float32_t fHl[KPLID][KSTAN];			//macierz obserwacji wysokości z lidaru
static float32_t fHm[KPMAP][KSTAN];			//macierz obserwacji wysokości z mapy
static float32_t fKc1[KSTAN][KPCIS];		//macierz wzmocnień Kalmana dla czujnika ciśnienia 1
static float32_t fKc2[KSTAN][KPCIS];		//macierz wzmocnień Kalmana dla czujnika ciśnienia 2
static float32_t fKa1[KSTAN][KPACC];		//macierz wzmocnień Kalmana dla akcelerometru 1
static float32_t fKa2[KSTAN][KPACC];		//macierz wzmocnień Kalmana dla akcelerometru 1
static float32_t fKg1[KSTAN][KPGNS];		//macierz wzmocnień Kalmana dla GNSS 1
static float32_t fKg2[KSTAN][KPGNS];		//macierz wzmocnień Kalmana dla GNSS 2
static float32_t fKl[KSTAN][KPLID];			//macierz wzmocnień Kalmana dla lidaru
static float32_t fKm[KSTAN][KPMAP];			//macierz wzmocnień Kalmana dla mapy
...
static arm_matrix_instance_f32 mZc  = {KPCIS, 1, fZc};					//wektor pomiaru: wysokość, prędkość z czujnika ciśnienia
static arm_matrix_instance_f32 mZa  = {KPACC, 1, fZa};					//wektor pomiaru:  przyspieszenie z akcelerometru
static arm_matrix_instance_f32 mZg  = {KPGNS, 1, fZg};					//wektor pomiaru:  wysokość z GNSS
static arm_matrix_instance_f32 mZl  = {KPLID, 1, fZl};					//wektor pomiaru:  wysokość z lidaru
static arm_matrix_instance_f32 mZm  = {KPMAP, 1, fZm};					//wektor pomiaru:  wysokość z mapy
...
static arm_matrix_instance_f32 mHc1 = {KPCIS, KSTAN, &fHc1[0][0]};		//macierz obserwacji wysokości i prędkości z czujnika ciśnienia 1
static arm_matrix_instance_f32 mHc2 = {KPCIS, KSTAN, &fHc2[0][0]};		//macierz obserwacji wysokości i prędkości z czujnika ciśnienia 2
...
static arm_matrix_instance_f32 mKg1 = {KSTAN, KPGNS, &fKg1[0][0]};		//macierz wzmocnień Kalmana GNSS 1
static arm_matrix_instance_f32 mKg2 = {KSTAN, KPGNS, &fKg2[0][0]};		//macierz wzmocnień Kalmana GNSS 2
static arm_matrix_instance_f32 mKl  = {KSTAN, KPLID, &fKl[0][0]};		//macierz wzmocnień Kalmana lidaru
static arm_matrix_instance_f32 mKm  = {KSTAN, KPMAP, &fKm[0][0]};		//macierz wzmocnień Kalmana mapy
...
uint8_t InicjujFiltrKalmanaWysokości10X10Z(stWymianyCM4_t *dane)
{
	...
	fRc1[0][0] = 0.056;		//wariancja statycznego pomiaru wysokości czujnika 1 [m^2]
	fRc1[1][1] = 0.407;		//wariancja statycznego pomiaru prędkości wariometru 1 [m^2/s^2]
	fRc2[0][0] = 0.0093;	//wariancja statycznego pomiaru wysokości czujnika 3 [m^2]
	fRc2[1][1] = 0.0016;	//wariancja statycznego pomiaru prędkości wariometru 3 [m^2/s^2]
	fRa1[0][0] = 0.031;		//wariancja statycznego pomiaru przyspieszenia akcelerometru 1 [m^2/s^4]
	fRa2[0][0] = 0.045;		//wariancja statycznego pomiaru przyspieszenia akcelerometru 2 [m^2/s^4]
...
uint8_t PredykcjaFiltraKalmanaWysokości10X10Z(stWymianyCM4_t *dane)
{
	...
	fQ[9][9] = fOkresPetli * WARIANCJA_ZMIANY_WYSOKOSCI_MAPY;
...
uint8_t AktulizacjaCzujnikiemCiśnienia1FiltraKalmanaWysokości10X10Z(stWymianyCM4_t *dane)
{
	...
	dane->stKalmanWys.fK[0] = fKc1[0][0];	//wpływu wysokości czujnka 1 na estymowaną wysokość
	dane->stKalmanWys.fK[2] = fKc1[1][1];	//wpływu prędkości czujnka 1 na estymowaną prędkość
...
uint8_t AktulizacjaCzujnikiemCiśnienia2FiltraKalmanaWysokości10X10Z(stWymianyCM4_t *dane)
{
	...
	fZc[0] = dane->fWysokoMSL[2];	//wysokość
	fZc[1] = dane->fWariometr[2];	//prędkość pionowa
	...
	dane->stKalmanWys.fK[1] = fKc2[0][0];	//wpływ wysokości czujnka 2 na estymowaną wysokość
	dane->stKalmanWys.fK[3] = fKc1[1][1];	//wpływu prędkości czujnka 2 na estymowaną prędkość
...
uint8_t AktulizacjaAkcelerometrem1FiltraKalmanaWysokości10X10Z(stWymianyCM4_t *dane)
{
	...
	dane->stKalmanWys.fK[4] = fKa1[2][0];	//wpływ akcelrometru 1 na estymowane przyspieszenie
...
uint8_t AktulizacjaAkcelerometrem2FiltraKalmanaWysokości10X10Z(stWymianyCM4_t *dane)
{
	...
	dane->stKalmanWys.fK[5] = fKa2[2][0];	//wpływ akcelrometru 2 na estymowane przyspieszenie
...
uint8_t AktulizacjaGNSS1FiltraKalmanaWysokości10X10Z(stWymianyCM4_t *dane)
{
	...
	//sprawdź czy pomiar mieści się w akceptowalnym zakresie
	fZg[0] = dane->stGnss[0].fWysokoscElips;		//pomiar wysokość elipsoidy. Zrobić: konwersję na wysokość n.p.m
	...
	dane->stKalmanWys.fK[6] = fKg1[0][0];	//wpływ GNSS1  na estymowaną wysokość
...
uint8_t AktulizacjaGNSS2FiltraKalmanaWysokości10X10Z(stWymianyCM4_t *dane)
{
	...
	//sprawdź czy pomiar mieści się w akceptowalnym zakresie
	fZg[0] = dane->stGnss[1].fWysokoscElips;		//pomiar wysokość elipsoidy. Zrobić: konwersję na wysokość n.p.m
	if ((fZg[0] < MIN_WYSOKOSC) || (fZg[0] > MAX_WYSOKOSC))
		return BLAD_ZLE_DANE;
	...
	dane->stKalmanWys.fK[7] = fKg2[0][0];	//wpływ GNSS1  na estymowaną wysokość
...
uint8_t AktulizacjaLidaremFiltraKalmanaWysokości10X10Z(stWymianyCM4_t *dane)
{
	...
	dane->stKalmanWys.fK[8] = fKl[0][0];	//wpływ wysokości z lidaru na estymowaną wysokość
...
uint8_t AktulizacjaMapąFiltraKalmanaWysokości10X10Z(stWymianyCM4_t *dane)
{
	...
	dane->stKalmanWys.fK[9] = fKm[0][0];	//wpływ wysokości z mapy na estymowaną wysokość
