...
uint8_t InicjujIIS2MDC(void)
{
	...
	chErr = HAL_I2C_Master_Transmit(&hi2c4, IIS2MDC_I2C_ADR, (uint8_t*)chDaneMagIIS, 1, 2);	//wyślij polecenie odczytu rejestru identyfikacyjnego
	if (!chErr)
	{
		chErr =  HAL_I2C_Master_Receive(&hi2c4, IIS2MDC_I2C_ADR + READ, (uint8_t*)chDaneMagIIS, 1, 2);		//odczytaj dane
		if (!chErr)
		{
			if (chDaneMagIIS[0] == 0x40)
			{
				...
				chErr = HAL_I2C_Master_Transmit(&hi2c4, IIS2MDC_I2C_ADR, (uint8_t*)chDaneMagIIS, 2, 2);	//wyślij polecenie zapisu konfiguracji
...
uint8_t StartujOdczytIIS2MDC(void)
{
	...
	if (uDaneCM4.dane.nZainicjowano & INIT_IIS2MDC)
	{
		...
		chErr = HAL_I2C_Master_Transmit_DMA(&hi2c4, IIS2MDC_I2C_ADR, (uint8_t*)chDaneMagIIS, 1);	//wyślij polecenie odczytu wszystkich pomiarów
	}
	return chErr;
}
...
uint8_t CzytajIIS2MDC(void)
{
	...
	if (uDaneCM4.dane.nZainicjowano & INIT_IIS2MDC)
	{
		...
		chErr = HAL_I2C_Master_Receive_DMA(&hi2c4, IIS2MDC_I2C_ADR + READ, (uint8_t*)chDaneMagIIS, 8);		//odczytaj dane
	}
	else
