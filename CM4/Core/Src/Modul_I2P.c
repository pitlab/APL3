...
float fMnożnikŻyro1[3], fMnożnikŻyro2[3];
...
uint8_t InicjujModulI2P(void)
{
	...
	do
	{
		...
		for (uint16_t n=0; n<3; n++)
		{
			cBłąd |= CzytajFramFloatZWalidacja(FAH_ZYRO1_X_PRZ_ZIM+(4*n), &fOffsetZyro1Z, VMIN_PRZES_ZYRO, VMAX_PRZES_ZYRO, VDOM_PRZES_ZYRO);		//offset żyroskopu1 na zimno
			cBłąd |= CzytajFramFloatZWalidacja(FAH_ZYRO2_X_PRZ_ZIM+(4*n), &fOffsetZyro2Z, VMIN_PRZES_ZYRO, VMAX_PRZES_ZYRO, VDOM_PRZES_ZYRO);		//offset żyroskopu2 na zimno

			cBłąd |= CzytajFramFloatZWalidacja(FAH_ZYRO1_X_PRZ_POK+(4*n), &fOffsetZyro1P, VMIN_PRZES_ZYRO, VMAX_PRZES_ZYRO, VDOM_PRZES_ZYRO);		//offset żyroskopu1 w temp pokojowej
			cBłąd |= CzytajFramFloatZWalidacja(FAH_ZYRO2_X_PRZ_POK+(4*n), &fOffsetZyro2P, VMIN_PRZES_ZYRO, VMAX_PRZES_ZYRO, VDOM_PRZES_ZYRO);		//offset żyroskopu2 w temp pokojowej

			cBłąd |= CzytajFramFloatZWalidacja(FAH_ZYRO1_X_PRZ_GOR+(4*n), &fOffsetZyro1G, VMIN_PRZES_ZYRO, VMAX_PRZES_ZYRO, VDOM_PRZES_ZYRO);		//offset żyroskopu1 na gorąco
			cBłąd |= CzytajFramFloatZWalidacja(FAH_ZYRO2_X_PRZ_GOR+(4*n), &fOffsetZyro2G, VMIN_PRZES_ZYRO, VMAX_PRZES_ZYRO, VDOM_PRZES_ZYRO);		//offset żyroskopu2 na gorąco

			cBłąd |= ObliczRownanieFunkcjiTemperatury(fOffsetZyro1Z, fOffsetZyro1P, fTemp1[ZIM], fTemp1[POK], &stWspKalTempZyro1.fAzim[n], &stWspKalTempZyro1.fBzim[n]);	//Żyro 1 na zimno
			cBłąd |= ObliczRownanieFunkcjiTemperatury(fOffsetZyro1P, fOffsetZyro1G, fTemp1[POK], fTemp1[GOR], &stWspKalTempZyro1.fAgor[n], &stWspKalTempZyro1.fBgor[n]);	//Żyro 1 na gorąco
			cBłąd |= ObliczRownanieFunkcjiTemperatury(fOffsetZyro2Z, fOffsetZyro2P, fTemp2[ZIM], fTemp2[POK], &stWspKalTempZyro2.fAzim[n], &stWspKalTempZyro2.fBzim[n]);	//Żyro 2 na zimno
			cBłąd |= ObliczRownanieFunkcjiTemperatury(fOffsetZyro2P, fOffsetZyro2G, fTemp2[POK], fTemp2[GOR], &stWspKalTempZyro2.fAgor[n], &stWspKalTempZyro2.fBgor[n]);	//Żyro 2 na gorąco

			cBłąd |= CzytajFramFloatZWalidacja(FAH_ZYRO1P_WZMOC+(4*n), &fMnożnikŻyro1[n], VMIN_MNOZNIK_ZYRO, VMAX_MNOZNIK_ZYRO, VDOM_MNOZNIK_ZYRO);	//wzmocnienie osi żyroskopu 1
			cBłąd |= CzytajFramFloatZWalidacja(FAH_ZYRO2P_WZMOC+(4*n), &fMnożnikŻyro2[n], VMIN_MNOZNIK_ZYRO, VMAX_MNOZNIK_ZYRO, VDOM_MNOZNIK_ZYRO);	//wzmocnienie osi żyroskopu 2
		...
		cBłąd |= CzytajFramFloatZWalidacja(FAH_CISN_ROZN1_POK, &fOffsetCisnRoz1P, VMIN_SKLADNIK_PDIF, VMAX_SKLADNIK_PDIF, VDOM_SKLADNIK_PDIF);		//offset czujnika różnicowego 1 w temp pokojowej
		cBłąd |= CzytajFramFloatZWalidacja(FAH_CISN_ROZN1_GOR, &fOffsetCisnRoz1G, VMIN_SKLADNIK_PDIF, VMAX_SKLADNIK_PDIF, VDOM_SKLADNIK_PDIF);		//offset czujnika różnicowego 1 na gorąco
		cBłąd |= ObliczRownanieFunkcjiTemperatury(fOffsetCisnRoz1Z, fOffsetCisnRoz1P, fTemp1[ZIM], fTemp1[POK], &stWspKalTempCzujnRozn1.fAzim, &stWspKalTempCzujnRozn1.fBzim);	//czujnik cisnienia różnicowego 1 na zimno
		cBłąd |= ObliczRownanieFunkcjiTemperatury(fOffsetCisnRoz1P, fOffsetCisnRoz1G, fTemp1[POK], fTemp1[GOR], &stWspKalTempCzujnRozn1.fAgor, &stWspKalTempCzujnRozn1.fBgor);	//czujnik cisnienia różnicowego 1 na gorąco

		cBłąd |= CzytajFramFloatZWalidacja(FAH_CISN_ROZN2_ZIM, &fOffsetCisnRoz2Z, VMIN_SKLADNIK_PDIF, VMAX_SKLADNIK_PDIF, VDOM_SKLADNIK_PDIF);		//offset czujnika różnicowego 2 na zimno
		cBłąd |= CzytajFramFloatZWalidacja(FAH_CISN_ROZN2_POK, &fOffsetCisnRoz2P, VMIN_SKLADNIK_PDIF, VMAX_SKLADNIK_PDIF, VDOM_SKLADNIK_PDIF);		//offset czujnika różnicowego 2 w temp pokojowej
		cBłąd |= CzytajFramFloatZWalidacja(FAH_CISN_ROZN2_GOR, &fOffsetCisnRoz2G, VMIN_SKLADNIK_PDIF, VMAX_SKLADNIK_PDIF, VDOM_SKLADNIK_PDIF);		//offset czujnika różnicowego 2 na gorąco
		cBłąd |= ObliczRownanieFunkcjiTemperatury(fOffsetCisnRoz2Z, fOffsetCisnRoz2P, fTemp1[ZIM], fTemp1[POK], &stWspKalTempCzujnRozn2.fAzim, &stWspKalTempCzujnRozn2.fBzim);	//czujnik cisnienia różnicowego 2 na zimno
		cBłąd |= ObliczRownanieFunkcjiTemperatury(fOffsetCisnRoz2P, fOffsetCisnRoz2G, fTemp1[POK], fTemp1[GOR], &stWspKalTempCzujnRozn2.fAgor, &stWspKalTempCzujnRozn2.fBgor);	//czujnik cisnienia różnicowego 2 na gorąco
...
uint8_t RozpocznijKalibracjeZeraZyroskopu(uint8_t chRodzajKalib)
{
	...
	uDaneCM4.dane.uRozne.U8[ODPOWIEDZ_U8] = BLAD_OK;
...
uint8_t KalibracjaWzmocnieniaZyro(uint8_t chRodzajKalib)
{
	...
	case POL7_KALIBRUJ_ZYRO_WZMP:	//kalibruj wzmocnienia żyroskopów P
		...
		{
			fMnożnikŻyro1[0] = OBR_KAL_WZM * 2 * M_PI  / fabsf(uDaneCM4.dane.fKatZyro1[0]);
			if ((fMnożnikŻyro1[0] > VMIN_MNOZNIK_ZYRO) && (fMnożnikŻyro1[0] < VMAX_MNOZNIK_ZYRO))
			{
				ZapiszFramFloat(FAH_ZYRO1P_WZMOC, fMnożnikŻyro1[0]);
				uDaneCM4.dane.nZainicjowano |= INIT_WYK_KAL_WZM_ZYRO;
				uDaneCM4.dane.uRozne.f32[0] = fMnożnikŻyro1[0];	//przekaż wartości kalibracji
			}
			else
			{
			...
			fMnożnikŻyro2[0] = OBR_KAL_WZM * 2 * M_PI  / fabsf(uDaneCM4.dane.fKatZyro2[0]);
			if ((fMnożnikŻyro2[0] > VMIN_MNOZNIK_ZYRO) && (fMnożnikŻyro2[0] < VMAX_MNOZNIK_ZYRO))
			{
				ZapiszFramFloat(FAH_ZYRO2P_WZMOC, fMnożnikŻyro2[0]);
				uDaneCM4.dane.nZainicjowano |= INIT_WYK_KAL_WZM_ZYRO;
				uDaneCM4.dane.uRozne.f32[1] = fMnożnikŻyro2[0];
			}
			else
			{
				uDaneCM4.dane.uRozne.U8[ODPOWIEDZ_U8] = ERR_ZLE_OBLICZENIA;
	...
	case POL7_KALIBRUJ_ZYRO_WZMQ:	//kalibruj wzmocnienia żyroskopów Q
		if ((uDaneCM4.dane.nZainicjowano & INIT_WYK_KAL_WZM_ZYRO) != INIT_WYK_KAL_WZM_ZYRO)
		{
			fMnożnikŻyro1[1] = OBR_KAL_WZM * 2 * M_PI  / fabsf(uDaneCM4.dane.fKatZyro1[1]);
			if ((fMnożnikŻyro1[1] > VMIN_MNOZNIK_ZYRO) && (fMnożnikŻyro1[1] < VMAX_MNOZNIK_ZYRO))
			{
				ZapiszFramFloat(FAH_ZYRO1Q_WZMOC, fMnożnikŻyro1[1]);
				uDaneCM4.dane.nZainicjowano |= INIT_WYK_KAL_WZM_ZYRO;
				uDaneCM4.dane.uRozne.f32[0] = fMnożnikŻyro1[1];	//przekaż wartości kalibracji
			}
			else
			{
				uDaneCM4.dane.uRozne.U8[ODPOWIEDZ_U8] = ERR_ZLE_OBLICZENIA;
			...
			fMnożnikŻyro1[1] = OBR_KAL_WZM * 2 * M_PI  / fabsf(uDaneCM4.dane.fKatZyro2[1]);
			if ((fMnożnikŻyro1[1] > VMIN_MNOZNIK_ZYRO) && (fMnożnikŻyro1[1] < VMAX_MNOZNIK_ZYRO))
			{
				ZapiszFramFloat(FAH_ZYRO2Q_WZMOC, fMnożnikŻyro1[1]);
				uDaneCM4.dane.nZainicjowano |= INIT_WYK_KAL_WZM_ZYRO;
				uDaneCM4.dane.uRozne.f32[1] = fMnożnikŻyro1[1];
			}
			else
			{
				uDaneCM4.dane.uRozne.U8[ODPOWIEDZ_U8] = ERR_ZLE_OBLICZENIA;
	...
	case POL7_KALIBRUJ_ZYRO_WZMR:	//kalibruj wzmocnienia żyroskopów R
		if ((uDaneCM4.dane.nZainicjowano & INIT_WYK_KAL_WZM_ZYRO) != INIT_WYK_KAL_WZM_ZYRO)
		{
			fMnożnikŻyro1[2] = OBR_KAL_WZM * 2 * M_PI  / fabsf(uDaneCM4.dane.fKatZyro1[2]);
			if ((fMnożnikŻyro1[2] > VMIN_MNOZNIK_ZYRO) && (fMnożnikŻyro1[2] < VMAX_MNOZNIK_ZYRO))
			{
				ZapiszFramFloat(FAH_ZYRO1R_WZMOC, fMnożnikŻyro1[2]);
				uDaneCM4.dane.nZainicjowano |= INIT_WYK_KAL_WZM_ZYRO;
				uDaneCM4.dane.uRozne.f32[0] = fMnożnikŻyro1[2];	//przekaż wartości kalibracji
			}
			else
			{
				uDaneCM4.dane.uRozne.U8[ODPOWIEDZ_U8] = ERR_ZLE_OBLICZENIA;
			...
			fMnożnikŻyro2[2] = OBR_KAL_WZM * 2 * M_PI  / fabsf(uDaneCM4.dane.fKatZyro2[2]);
			if ((fMnożnikŻyro2[2] > VMIN_MNOZNIK_ZYRO) && (fMnożnikŻyro2[2] < VMAX_MNOZNIK_ZYRO))
			{
				ZapiszFramFloat(FAH_ZYRO2R_WZMOC, fMnożnikŻyro2[2]);
				uDaneCM4.dane.nZainicjowano |= INIT_WYK_KAL_WZM_ZYRO;
				uDaneCM4.dane.uRozne.f32[1] = fMnożnikŻyro2[2];
			}
			else
			{
				uDaneCM4.dane.uRozne.U8[ODPOWIEDZ_U8] = ERR_ZLE_OBLICZENIA;
...
uint8_t KalibrujCisnienie(float fCisnienie1, float fCisnienie2, float fTemp, uint16_t sLicznik, uint8_t chPrzebieg)
{
	...
	if (chPrzebieg == 0xFF)
	{
		...
		for (uint8_t n=0; n<LICZBA_CZUJ_CISN; n++)
			uDaneCM4.dane.uRozne.f32[n+4] = fMnożnikCiśn[n];	//ustaw bieżący współczynnik skalowania
	...
	if (sLicznik < CZAS_KALIBRACJI)
	{
		...
		cBłąd = ERR_GOTOWE;		//jeżeli zebrano tyle co trzeba to przestań sumować i zwróć kod zakończenia
	...
	if ((sLicznik == CZAS_KALIBRACJI - 1) && chPrzebieg)
	{
		...
		float fRoboczyMnożnikCiśn[LICZBA_CZUJ_CISN];
		...
		for (uint8_t n=0; n<LICZBA_CZUJ_CISN; n++)
		{
			...
			if (fSredCisn1[n] > fSredCisn2[n])
			{
				...
				fRoboczyMnożnikCiśn[n] = fCisnWzorcowe[n] / fSredCisn2[n];
			}
			else
			{
				fCisnWzorcowe[n] = CisnienieBarometryczne(WYSOKOSC10PIETER, fSredCisn2[n], fTemp);
				fRoboczyMnożnikCiśn[n] = fCisnWzorcowe[n] / fSredCisn1[n];
			}
...
uint8_t ZapiszKalibracjeMagnetometru(uint8_t chMagn)
{
	...
	for (uint16_t n=0; n<3; n++)
	{
		...
		case MAG3:
			...
			ZapiszFramFloat(FAH_MAGN3_SKLADNIK_X + 4*n, fPrzesMagn3[n]);
