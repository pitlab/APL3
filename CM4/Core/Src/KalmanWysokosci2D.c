...
#include <KalmanWysokosci2D.h>
 ...
 float32_t fX[2];		//wektor stanu: 0=wysokość, 1=prędkość
...
uint8_t InicjujFiltrKalmanaWysokości2D(void)
{
	...
	//pierwsza estymata wektora stanu ma wartość pomiaru
	fX[0] = fZ[0];
	fX[1] = fZ[1];
...
uint8_t PredykcjaFiltraKalmanaWysokości2D(stWymianyCM4_t *dane)
{
	...
	cBłąd |= arm_mat_add_f32(&mTempW3, &mW, &mX);
...
uint8_t AktulizacjaFiltraKalmanaWysokości2D(stWymianyCM4_t *dane)
{
	...
	cBłąd |= arm_mat_mult_f32(&mH, &mX, &mTempW1);
	...
	if ((fTempW2[0] < dane->stBSP.fWysokoscMSL + 200.0f) && (fTempW2[0] > dane->stBSP.fWysokoscMSL - 200.0f)) //nie przepisuj wyników mocno odbiegajacych od wartosci zmiennej
		dane->stBSP.fWysokoscMSL = fX[0] = fTempW2[0];
	if ((fTempW2[1] < dane->stBSP.fPredkoscD + 2.0f) && (fTempW2[1] > dane->stBSP.fPredkoscD - 2.0f))
