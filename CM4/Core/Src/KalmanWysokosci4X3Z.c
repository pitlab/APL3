...
static float32_t fZ[3] = {0};		//wektor pomiaru: 0=wysokość, 1=przyspieszenie
static float32_t fF[4][4];			//macierz przejścia wektora stanu
static float32_t fP[4][4];			//macierz kowariancji predykcji
static float32_t fR[3][3];			//macierz kowariancji pomiaru
static float32_t fQ[4][4];			//macierz szumu procesu
static float32_t fI[4][4];			//macierz jednostkowa
static float32_t fHh[3][4];			//macierz obserwacji przyspieszenia, prędkości i wysokości, przekształca jednostkę pomiaru w jednostkę wektora stanu
static float32_t fHa[3][4];			//macierz obserwacji przyspieszenia, przekształca jednostkę pomiaru w jednostkę wektora stanu
static float32_t fK[4][3];			//macierz wzmocnień kalnama
static float32_t fPHt[4][3];		//macierz 4x2 na wyniki pośrednie
static float32_t fTempM43[4][3];	//macierz 4x2 na wyniki pośrednie
static float32_t fTempM34[3][4];	//macierz 2x4 na wyniki pośrednie
static float32_t fTempM44A[4][4];
static float32_t fTempM44B[4][4];
static float32_t fTempM44C[4][4];
static float32_t fTempM33A[3][3];
static float32_t fTempM33B[3][3];
static float32_t fTempM31A[3];
static float32_t fTempM31B[3];
...
static arm_matrix_instance_f32 mZ  = {3, 1, fZ};			//wektor pomiaru: wysokość, prędkość, przyspieszenie
...
static arm_matrix_instance_f32 mR  = {3, 3, &fR[0][0]};		//macierz kowariancji pomiaru
...
static arm_matrix_instance_f32 mHh = {3, 4, &fHh[0][0]};	//macierz obserwacji wysokości
static arm_matrix_instance_f32 mHa = {3, 4, &fHa[0][0]};	//macierz obserwacji przyspieszenia
static arm_matrix_instance_f32 mK  = {4, 3, &fK[0][0]};		//macierz wzmocnień kalmana
static arm_matrix_instance_f32 mPHt = {4, 3, &fPHt[0][0]};	//macierz 4x3 na wyniki pośrednie
static arm_matrix_instance_f32 mTempM43  = {4, 3, &fTempM43[0][0]};	//macierz 4x3 na wyniki pośrednie
static arm_matrix_instance_f32 mTempM34  = {3, 4, &fTempM34[0][0]};	//macierz 3x4 na wyniki pośrednie
...
static arm_matrix_instance_f32 mTempM33A = {3, 3, &fTempM33A[0][0]};	//macierz 3x3 na wyniki pośrednie
static arm_matrix_instance_f32 mTempM33B = {3, 3, &fTempM33B[0][0]};	//macierz 3x3 na wyniki pośrednie
static arm_matrix_instance_f32 mTempM31A = {3, 1, fTempM31A};	//macierz 2x1 na wyniki pośrednie
static arm_matrix_instance_f32 mTempM31B = {3, 1, fTempM31B};	//macierz 2x1 na wyniki pośrednie
...
uint8_t InicjujFiltrKalmanaWysokości4X3Z(stWymianyCM4_t *dane)
{
	...
	fX[0] = fZ[0];		//wysokość
	fX[1] = fZ[1];		//prędkość pionowa
	fX[2] = 0.00001;	//przyspieszenie kinematyczne
	fX[3] = fZ[2];		//łączne przyspieszenie w osi Z: grawitacja + bias
	arm_mat_init_f32(&mX, 4, 1, fX);
	arm_mat_init_f32(&mZ, 3, 1, fZ);
	...
	fR[1][1] = 2.097;	//wariancja statycznego pomiaru prędkości wariometru 1 [m^2/s^2]
	fR[2][2] = 0.399;	//wariancja statycznego pomiaru przyspieszenia akcelerometru 1[m^2/s^4]

	arm_mat_init_f32(&mR, 3, 3, &fR[0][0]);
	...
	fHh[2][2] = 1.0f;
	fHh[2][3] = 1.0f;
	arm_mat_init_f32(&mHh, 3, 4, &fHh[0][0]);

	fHa[2][2] = 1.0f;
	fHa[2][3] = 1.0f;
	arm_mat_init_f32(&mHa, 3, 4, &fHa[0][0]);
	return cBłąd;
}
