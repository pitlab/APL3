...
const int8_t cZnakZyro[3] = {-1, 1, -1};	//korekcja znaku prędkości żyroskopów
const int8_t cZnakAkcel[3] = {-1, 1, 1};	//korekcja znaku akcelerometrów
...
uint8_t InicjujICM42688(void)
{
	...
	cDane[0] = CzytajSPIu8(PICM4268_WHO_I_AM);		//sprawdź obecność układu
	if (cDane[0] != 0xDB)
		return BLAD_BRAK_CZUJNIKA;


	//włącz żyroskopu i akcelerometry w tryb Low Noise
	cDane[0] = PICM4268_PWR_MGMT0;
	cDane[1] = (0 << 5) |	//TEMP_DIS. 0: Temperature sensor is enabled (default), 1: Temperature sensor is disabled
	...
	cDane[0] = PICM4268_GYRO_CONFIG0;
	cDane[1] = (1 << 5) |	//GYRO_FS_SEL: 000: ±2000 dps (default), 001: ±1000 dps, 010: ±500 dps, 011: ±250 dps, 100: ±125 dps, 101: ±62.5 dps, 110: ±31.25 dps, 111: ±15.625 dps
	...
	cDane[0] = PICM4268_ACCEL_CONGIG0;
	cDane[1] = (1 << 5) |	//ACCEL_FS_SEL: 000: ±16g (default), 001: ±8g, 010: ±4g, 011: ±2g, 100: Reserved, 101: Reserved, 110: Reserved, 111: Reserved
	...
	//przełącz na bank 1
	cDane[0] = PICM4268_BANK_SEL;
	cDane[0] = 1;
	ZapiszSPIu8(cDane, 2);

	//wyłącz filtry: nothch i AAF na ścieżce żyroskopów
	cDane[0] = PICM4268_B1_GYRO_CONFIG_STATIC2;
	cDane[1] = (1 << 1) |	//GYRO_AAF_DIS: 0: Enable gyroscope anti-aliasing filter (default), 1: Disable gyroscope anti-aliasing filter
	...
	cDane[0] = PICM4268_B2_ACCEL_CONFIG_STATIC2;
	cDane[1] = (0 << 1) |	//ACCEL_AAF_DELT: Controls bandwidth of the accelerometer anti-alias filter
...
uint8_t ObslugaICM42688(void)
{
	...
	{
		cDane[0] = PICM4268_TEMP_DATA1 | READ_SPI;
		HAL_GPIO_WritePin(MOD_SPI_NCS_GPIO_Port, MOD_SPI_NCS_Pin, GPIO_PIN_RESET);	//CS = 0
		HAL_SPI_TransmitReceive(&hspi2, cDane, cDane, 15, TOUT_SPI);
		HAL_GPIO_WritePin(MOD_SPI_NCS_GPIO_Port, MOD_SPI_NCS_Pin, GPIO_PIN_SET);	//CS = 1

		uDaneCM4.dane.fTemper[TEMP_IMU1] = (float)((int16_t)((cDane[1] <<8) + cDane[2]) / 132.48) + 25.0 + KELVIN;	//temperatura w K
		...
		{
			uDaneCM4.dane.fAkcel1[n] = (float)((int16_t)(cDane[2*n+3] <<8) + cDane[2*n+4]) * (8.0 * AKCEL1G / 32768.0) * cZnakAkcel[n];			//+-8g -> [m/s^2]
			uDaneCM4.dane.fZyroSur1[n] = (float)((int16_t)(cDane[2*n+9] <<8) + cDane[2*n+10]) * (1000.0 * DEG2RAD / 32768.0) * cZnakZyro[n];	//+-1000°/s -> [rad/s]
