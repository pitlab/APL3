...
HAL_StatusTypeDef PobierzDateCzas(RTC_DateTypeDef *sData, RTC_TimeTypeDef *sCzas);
uint32_t PobierzCzasT6(void);
uint32_t MinalCzas(uint32_t nPoczatek);
uint32_t MinalCzas2(uint32_t nPoczatek, uint32_t nKoniec);
uint8_t CzekajzTimeoutemPókiZajęty(uint8_t chZajety, uint32_t nCzasOczekiwania);
