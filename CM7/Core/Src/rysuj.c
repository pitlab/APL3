...
uint8_t Menu(char *tytul, menu_t *menu, uint8_t *cPozycjaMenu)
{
	...
	*cPozycjaMenu = 0;
	...
	if (stTime.Seconds != cOstatniCzas)
	{
		...
		//błędy przekazywane z CM4 odświeżaj razez czasem co sekundę
		switch (uDaneCM4.dane.cBuforBłędów[cIndeksBuforaBłędów])
		{
		case BLAD_OK: 				sprintf(cNapis, "B:OK    ");	break;
		case BLAD_HAL: 				sprintf(cNapis, "B:HALTim");	break;
		...
		case BLAD_BRAK_DANYCH:		sprintf(cNapis, "B:BrakDa");	break;
		case BLAD_ZLY_STAN_PROT:	sprintf(cNapis, "B:Protok");	break;
		case BLAD_ZLE_POLECENIE:	sprintf(cNapis, "B:Polece");	break;
		case BLAD_ZLY_INTERFEJS:	sprintf(cNapis, "B:Interf");	break;
		case BLAD_ZLE_DANE:			sprintf(cNapis, "B:zlDane");	break;
		...
		case BLAD_SEMAFOR_ZAJETY:	sprintf(cNapis, "B:Semafo");	break;
		...
		default:					sprintf(cNapis, "B:InnyBl");	break;
		}
