...
uint8_t SprawdzObecnoscFlashNOR(void)
{
	...
	cBłąd = HAL_NOR_Read_ID(&hnor3, &norID);
...
uint8_t KasujSektorFlashNOR(uint32_t nAdres)
{
	...
	cBłąd = HAL_NOR_Erase_Block(&hnor3, ADRES_NOR, nAdres & 0x00FFFFFF);	//potrzebny jest adres względny
	if (cBłąd == BLAD_OK)
		cBłąd = HAL_NOR_GetStatus(&hnor3, nAdres, 1100);	//Sector Erase time 128 kbyte (typ/max) = 275/1100ms (pdf str.46)
	return cBłąd;
}
...
{
	uint8_t cBłąd, chStan;

	chStan = HAL_NOR_GetState(&hnor3);
	if (chStan == HAL_NOR_STATE_PROTECTED)
	{
		cBłąd = HAL_NOR_WriteOperation_Enable(&hnor3);
		if (cBłąd)
			return cBłąd;
	}

	cBłąd = HAL_NOR_ProgramBuffer(&hnor3, nAdres & 0x00FFFFFF, sDane, nIlosc);	//potrzebny jest adres względny
	if (cBłąd == BLAD_OK)
		cBłąd = HAL_NOR_GetStatus(&hnor3, nAdres, 2);	//Buffer Programming time (Typ/max) = 340/750us  (pdf str.46)
