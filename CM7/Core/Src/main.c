 ...
 *Zrobić:
 - Dodać polecenie Blank check oraz ramkę komunikacyjną do tego
  ...
  - znaleźć dlaczego czasami w trakcie kalibracji magnetometru gaśnie wyświetlacz - po oberwaniu się i naprawie przewodu CS przestał gasnąć
  - ustawić odpowiednie kąty w poziomicy podczas kalibracji skalowania żyroskopów
  - poprawić podwójny kąt liczony IMU4
  - sprawdzić dlaczego w magnetometrze 2 źle rysują się wykresy dla obracania wokół osi Z i Y - zła kalibracja, oraz zerowania ekstremów
 * */
