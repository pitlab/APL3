...
DRESULT SDRAMDISK_read(BYTE lun, BYTE *buff, DWORD sector, UINT count)
{
  ...
  uint16_t *pSdramAddress = (uint16_t *) (SDRAM_DEVICE_ADDR + (sector * BLOCK_SIZE));


  for(; BufferSize != 0; BufferSize--)
  {
	  u1632.dane16[0] = *(__IO uint16_t *)pSdramAddress++;
	  u1632.dane16[1] = *(__IO uint16_t *)pSdramAddress++;
	  *pSrcBuffer++ = u1632.dane32;
...
DRESULT SDRAMDISK_write(BYTE lun, const BYTE *buff, DWORD sector, UINT count)
{
  ...
  uint32_t BufferSize = (BLOCK_SIZE * count)/4;
  uint16_t *pSramAddress = (uint16_t *) (SDRAM_DEVICE_ADDR + (sector * BLOCK_SIZE));

  for(; BufferSize != 0; BufferSize--)
  {
    //*(__IO uint32_t *)pSramAddress++ = *pDstBuffer++;
	  u1632.dane32 = *pDstBuffer++;
	 *(__IO uint16_t *)pSramAddress++ =  u1632.dane16[0];
	 *(__IO uint16_t *)pSramAddress++ =  u1632.dane16[1];
  }
